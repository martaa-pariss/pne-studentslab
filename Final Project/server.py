    ...
    def do_GET(self):
        ...
        #2: List
            ...
            if response.status == 200:
                ...
                contents = "Error connecting with Ensembl"
                error_code = 500

        #3: karyo
        elif resource == "/karyotype":
            specie = params.split("=")[1]
            conn = http.client.HTTPSConnection("rest.ensembl.org")
        ...
        #4: Long
            ...
            if response.status == 200:
                data = json.loads(response.read().decode())
                long = "not found"
                for region in data['top_level_region']: #searching in dif reg for the chromo name
                ...
                contents = Path('html/error.html').read_text().replace("{{message}}", "Error in the data")
                error_code = 404
...
#MEDIUM LEVEL#
        ...
        # CASO 6: /geneSeq dado nombre
            ...
            gene_name = params.split("=")[1] #input name
            conn = http.client.HTTPSConnection("rest.ensembl.org")
            conn.request("GET", "/lookup/symbol/homo_sapiens/" + gene_name, headers={"Content-Type": "application/json"})
            response = conn.getresponse()

            if response.status == 200: #AQUI TENGO QUE MIRAR PORQUE NO SE PORQUE TENGO Q OBTENER DOS RESPUESTAS DIFERENTES
                gene_id = json.loads(response.read().decode())['id'] #sacamos id xq necesitamos la secuencia
                ...
                contents = "Error found in sequence"
                error_code = 404
        ...
        #8: longitud porcentaje (tngo q volver a sacar id cn nombre para acceder a info...)
            ...
            if response.status == 200:
                ...
                contents = page.replace("{{gene}}", gene_name).replace("{{length}}", length).replace("{{percents}}", percents)
            else:
                contents = "Error obtaining the sequence"
                error_code = 404
        #9: /geneList (gnes en reg)
            ...
            for part in parts: #otra manera d partir cndo tngo muchos params xq sino me lio
